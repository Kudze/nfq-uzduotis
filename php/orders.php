...
class OrderManager {
    ...
    public static function renderOrdersList() {
       ...
       self::_handleOrderForm();
        ...
        $oSearchText = @$_GET['oSearch'];
        $oCurrPage = @$_GET['oPage'];
        $oMinPage = 1;
        $oItems = self::_fetchOrders(20, $oCurrPage, $oMaxPage, $oSearchText);
    ...
    private static function _fetchOrders($itemsPerPage, &$currPage, &$maxPage, $search) {
        $res = array();
        ...
        try {
            ...
            $stmt->execute(array("%" . $search . "%", 0, 20));

            while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                array_push($res, new Order($row));
            }
